#!/usr/bin/env bash
# sky130-magic-only.sh — Magic + XQuartz + Sky130 PDK with robust checks, auto-repair, and dual launchers (macOS)
# Launchers after install:
#   magic-sky130           # Magic GUI (via XQuartz, sane window)
#   magic-sky130 --safe    # Magic headless (no GUI)
#   magic-sky130-xsafe     # Magic GUI (hardened: software GL + XQuartz pref fix)
...
ensure_dir() {
  ...
  if [[ -n "$need_sudo" ]]; then sudo install -d -m 755 "$d"; else install -d -m 755 "$d"; fi
}
ensure_path_now(){ export PATH="$MACPORTS_PREFIX/bin:$MACPORTS_PREFIX/sbin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:$PATH"; }

# Presence detectors
magic_path(){ [[ -x /opt/local/bin/magic ]] && { echo /opt/local/bin/magic; return 0; }
             [[ -x /usr/local/bin/magic ]] && { echo /usr/local/bin/magic; return 0; }
             return 1; }
pdk_loc(){
  for base in "/opt/pdk" "/opt/pdk/share/pdk" "/usr/local/share/pdk"; do
    for name in sky130A sky130B; do
      if [[ -f "$base/$name/libs.tech/magic/${name}.magicrc" ]]; then echo "$base $name"; return 0; fi
    done
  done
  return 1
}
...
repair_xquartz(){
  ...
  # Harden preference to allow indirect GLX (helps some GPUs)
...
tk_wish_sanity() {
  ...
  launchctl setenv DISPLAY "$disp" >/dev/null 2>&1 || true
  /opt/X11/bin/xhost +SI:localuser:$USER >/dev/null 2>&1 || true
  local WISH="/opt/local/bin/wish8.6"; [[ -x "$WISH" ]] || WISH="/opt/local/bin/wish8.7"
  [[ -x "$WISH" ]] || return 1
  ensure_dir "$WORKDIR"
...
repair_tk_wish() {
  ...
  local line; line="$(/opt/local/bin/port -qv installed tk 2>/dev/null | awk '/active/{print}')"
  if [[ "$line" != *"+x11"* ]]; then
...
need_xcode() {
  ...
  if xcode-select -p >/dev/null 2>&1; then
    ...
    else mark_fail "Xcode Command Line Tools" "Not installed. Finish Apple installer, then re-run."; fi
  fi
}
fix_macports_signing() {
  ...
  while IFS= read -r -d '' f; do sudo /usr/bin/codesign --force --sign - "$f" >/dev/null 2>&1 || true; done \
    < <(/usr/bin/find /opt/local -type f \( -name 'tclsh*' -o -name '*.dylib' \) -print0)
}
macports_ok() { /usr/bin/env -i PATH="$MACPORTS_PREFIX/bin:$MACPORTS_PREFIX/sbin:/usr/bin:/bin:/usr/sbin:/sbin" HOME="$HOME" "$MACPORTS_PREFIX/bin/port" version >/dev/null 2>&1; }
...
ensure_macports() {
  say "Ensuring MacPorts…"; ensure_path_now
  if command -v port >/dev/null 2>&1; then
    if macports_ok; then sudo port -v selfupdate >/dev/null 2>&1 || true; mark_pass "MacPorts CLI ready"; return; fi
    fix_macports_signing; if macports_ok; then mark_pass "MacPorts (post-signing-fix) ready"; return; fi
    if reinstall_macports_source && macports_ok; then mark_pass "MacPorts (rebuilt from source)"; return; fi
    mark_fail "MacPorts" "port not usable even after source rebuild"; return
  fi
  ensure_dir "$WORKDIR"; cd "$WORKDIR" || exit 1
  local swmaj; swmaj="$(sw_vers -productVersion | awk -F. '{print $1}')"
  local PKG=""; case "$swmaj" in
    15) PKG="MacPorts-2.10.4-15-Sequoia.pkg" ;; 14) PKG="MacPorts-2.10.4-14-Sonoma.pkg" ;;
    13) PKG="MacPorts-2.10.4-13-Ventura.pkg" ;; 12) PKG="MacPorts-2.10.4-12-Monterey.pkg" ;;
  ...
  if curl -fL --retry 3 "https://distfiles.macports.org/MacPorts/${PKG}" -o "$PKG" && sudo installer -pkg "$PKG" -target /; then
    ...
    if sudo /opt/local/bin/port -v selfupdate >/dev/null 2>&1; then mark_pass "MacPorts installed"
      ...
      fix_macports_signing
      if sudo /opt/local/bin/port -v selfupdate >/dev/null 2>&1; then mark_pass "MacPorts installed (post-signing-fix)"
      else if reinstall_macports_source && macports_ok; then mark_pass "MacPorts (rebuilt from source)"; else mark_fail "MacPorts" "Selfupdate failed; source rebuild also failed"; fi
      fi
    fi
  else mark_fail "MacPorts" "Failed to download/install pkg"; fi
}
ensure_xquartz() {
  say "Ensuring XQuartz…"
  local have=false
  if [[ -d "/Applications/XQuartz.app" || -d "/Applications/Utilities/XQuartz.app" ]]; then have=true
  else
    ensure_dir "$WORKDIR"; cd "$WORKDIR" || exit 1
    local PKG_URL=""; if curl -fsSL "https://api.github.com/repos/XQuartz/XQuartz/releases/latest" -o xq.json; then
      PKG_URL="$(awk -F\" '/"browser_download_url":/ && /\.pkg"/ {print $4; exit}' xq.json)"
    fi
    if [[ -n "$PKG_URL" ]] && curl -fL "$PKG_URL" -o XQuartz.pkg && sudo installer -pkg XQuartz.pkg -target /; then have=true; fi
  fi
  if ! $have; then mark_fail "XQuartz" "App not installed"; return; fi
  # Preference that helps GLX on some GPUs
  defaults write org.xquartz.X11 enable_iglx -bool true >/dev/null 2>&1 || true
  if xquartz_sanity; then mark_pass "XQuartz running (DISPLAY=$(xquartz_display_value))"
  else
    ...
    else mark_fail "XQuartz" "Repair failed; GUI may crash"; fi
  fi
}
ports_install_magic() {
  say "Checking Magic…"
  if MAGIC_BIN="$(magic_path)"; then mark_pass "Magic present at ${MAGIC_BIN}"; return; fi
  ...
  if MAGIC_BIN="$(magic_path)"; then mark_pass "Magic installed at ${MAGIC_BIN}"; else mark_fail "Magic" "MacPorts install failed"; fi
}
install_pdk() {
  say "Checking Sky130 PDK…"
  if read -r PDK_BASE PDK_NAME < <(pdk_loc); then mark_pass "Sky130 PDK present at ${PDK_BASE}/${PDK_NAME}"; return; fi
  say "Installing Sky130 PDK via open_pdks…"
  ensure_dir "$PDK_PREFIX" sudo; sudo chown "$(id -u)":"$(id -g)" "$PDK_PREFIX" || true
  ensure_dir "$WORKDIR"; cd "$WORKDIR" || exit 1
  if [[ -d open_pdks/.git ]]; then (cd open_pdks && git pull --rebase >/dev/null 2>&1)
  else git clone https://github.com/RTimothyEdwards/open_pdks.git >/dev/null 2>&1 || { mark_fail "Sky130 PDK" "git clone failed"; return; }; fi
  cd open_pdks || { mark_fail "Sky130 PDK" "open_pdks dir missing"; return; }
  sudo port -N install git gawk wget tcl tk >/dev/null 2>&1 || true
  ./configure --prefix="$PDK_PREFIX" --enable-sky130-pdk --with-sky130-local-path="$PDK_PREFIX" --enable-sram-sky130 >/dev/null 2>&1 \
    && make -j"$(/usr/sbin/sysctl -n hw.ncpu 2>/dev/null || echo 4)" >/dev/null 2>&1 \
    && sudo make install >/dev/null 2>&1 || true
  if read -r PDK_BASE PDK_NAME < <(pdk_loc); then mark_pass "Sky130 PDK installed at ${PDK_BASE}/${PDK_NAME}"
  else mark_fail "Sky130 PDK" "open_pdks build/install failed; no sky130A/B found under /opt or /usr/local"; fi
}
write_demo() {
  say "Writing demo files…"; ensure_dir "$DEMO_DIR"
...
write_rc_wrapper() {
  say "Writing Magic rc wrapper…"; ensure_dir "$RC_DIR"
...
install_launchers() {
  say "Installing Magic launchers…"
  # Choose target dir: prefer /usr/local/bin, else /opt/local/bin
  local tdir="/usr/local/bin"
  if ! sudo install -d -m 755 "$tdir" 2>/dev/null; then
    warn "Could not create $tdir; falling back to /opt/local/bin."
    tdir="/opt/local/bin"; sudo install -d -m 755 "$tdir"
  fi

  # Normal launcher -----------------------------------------------------------
  local target="$tdir/magic-sky130"
...
if [[ "${1:-}" == "--safe" ]]; then exec "${CLEAN_ENV[@]}" "$MAGIC_BIN" -norcfile -dnull -noconsole -T "$PDK" -rcfile "$RC" "${@:2}"; fi
exec "${CLEAN_ENV[@]}" "$MAGIC_BIN" -norcfile -d X11 -T "$PDK" -rcfile "$RC" "$@"
EOF
  then
    sudo chmod +x "$target"
  else
    mark_fail "Magic launcher" "Could not write $target"
  fi

  # X-safe launcher (software GL + enable_iglx) ------------------------------
  local target2="$tdir/magic-sky130-xsafe"
  if sudo tee "$target2" >/dev/null <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
choose_pdk(){ for b in /opt/pdk /opt/pdk/share/pdk /usr/local/share/pdk; do for n in sky130A sky130B; do [[ -d "$b/$n" ]]&&{ echo "$b" "$n"; return 0; }; done; done; return 1; }
read -r PDK_ROOT PDK < <(choose_pdk) || { echo "No SKY130 PDK found."; exit 1; }
export PDK_ROOT PDK
# Harden XQuartz pref to allow indirect GLX (helps crashy GPUs)
defaults write org.xquartz.X11 enable_iglx -bool true >/dev/null 2>&1 || true
...
EOF
  then
    ...
    mark_fail "Magic xsafe launcher" "Could not write $target2"
  fi

  # Convenience symlinks if we fell back to /opt/local/bin
  if [[ "$tdir" = "/opt/local/bin" && -d /usr/local/bin ]]; then
    sudo ln -sf "$tdir/magic-sky130" /usr/local/bin/magic-sky130 || true
    sudo ln -sf "$tdir/magic-sky130-xsafe" /usr/local/bin/magic-sky130-xsafe || true
  fi
...
headless_check() {
  ...
  if ! MAGIC_BIN="$(magic_path)"; then mark_fail "Magic tech check" "magic binary not found"; return; fi
  if ! read -r PDK_BASE PDK_NAME < <(pdk_loc); then mark_fail "Magic tech check" "Sky130 PDK not found"; return; fi
  ensure_dir "$DEMO_DIR"
  cat > "$DEMO_DIR/smoke.tcl" <<'EOF'
puts ">>> smoke: tech=[tech name]"
quit -noprompt
EOF
  /usr/bin/env -i PATH="/opt/local/bin:/opt/local/sbin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin" \
  ...
  if grep -q ">>> smoke: tech=" "$HEADLESS_LOG" 2>/dev/null; then mark_pass "Magic headless tech load"
  else mark_fail "Magic headless tech load" "See $HEADLESS_LOG"; fi
}

summary() {
  echo; say "==== INSTALL SUMMARY ===="
  ((${#PASS[@]})) && { ok "Passed:"; for p in "${PASS[@]}"; do echo "  • $p"; done; }
  if ((${#FAIL[@]})); then
    err "Failed:"; for f in "${FAIL[@]}"; do echo "  • $f"; done
    echo; err "One or more checks failed. Review messages above."; exit 1
  else
    ...
    echo "  • magic-sky130-xsafe     (GUI, software GL — use on machines where XQuartz/Wish crashes)"
    echo
    echo "Demo:"
...
main() {
  ensure_dir "$WORKDIR"; ensure_dir "$RC_DIR"; ensure_dir "$DEMO_DIR"
  need_xcode
  ensure_macports
  ensure_path_now
  ensure_xquartz
  # Verify Tk/Wish; auto-repair once if it fails (prevents “wish quit unexpectedly”)
  if tk_wish_sanity; then mark_pass "Tk/Wish GUI sanity"
  else
    warn "Tk/Wish sanity failed; attempting repair…"
    repair_tk_wish
    if tk_wish_sanity; then mark_pass "Tk/Wish repaired"
    else mark_fail "Tk/Wish" "Wish still crashing; see MacPorts tk and XQuartz"; fi
  fi
  ports_install_magic
  install_pdk
  write_demo

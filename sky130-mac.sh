#!/bin/sh
# macOS SKY130 — One-Shot Installer (POSIX sh, robust, student-friendly)
# ----------------------------------------------------------------------
# After this finishes, users can just run:   magic &
# Key points:
#  • Builds Magic from source (Cocoa Tk) → avoids XQuartz/tk-x11 issues.
#  • Installs SKY130A via open_pdks, then verifies & auto-fixes PDK_ROOT.
#  • Puts a ~/bin/magic shim in front so "magic" resolves correctly everywhere.
...
install_brew_deps() {
  ...
  run "brew install --cask xquartz || true"  # harmless if using Cocoa Tk; helps if user later installs tk-x11 tools
  ok "Dependencies installed."
}

# ---- 3) Magic (build from source → Cocoa Tk, no XQuartz needed) ----
...
# Ensure `magic` is the command students get (prefer user build, add ~/bin shim)
ensure_magic_on_path() {
  USER_MAGIC="$MAGIC_PREFIX/bin/magic"
  if [ -x "$USER_MAGIC" ]; then
    ln -sf "$USER_MAGIC" "$HOME/bin/magic"
    case ":$PATH:" in *":$HOME/bin:"*) : ;; *) PATH="$HOME/bin:$PATH"; export PATH ;; esac
    hash -r 2>/dev/null || true
    ok "Using user-built magic via ~/bin/magic → $USER_MAGIC"
  else
    warn "User-built Magic not found at $USER_MAGIC (unexpected). PATH=$PATH"
  fi
}
...
# ---- 5) Locate PDK_ROOT (parent that CONTAINS sky130A/) ----
find_pdk_root() {
...
export MAGTYPE=mag
# Put our shims/tools first
export PATH="\$HOME/bin:$MAGIC_PREFIX/bin:\$PATH"
# Also allow MacPorts tools if present
[ -d "/opt/local/bin" ] && export PATH="/opt/local/bin:\$PATH"
# Homebrew Tcl/Tk flags (help Magic stability)
if command -v brew >/dev/null 2>&1; then
...
# Handy helper that never depends on the shell env
...
if [ -z "$PDK_ROOT_GUESS" ]; then
  for b in "$HOME/eda/pdks/share/pdk" /opt/pdk/share/pdk /usr/local/share/pdk /opt/homebrew/share/pdk /opt/local/share/pdk; do
...
# ---- 7) VERIFY + AUTO-FIX SKY130 LOADING (no MacPorts fallback) ----
ensure_sky130_loaded_and_fix() {
  ...
  # Candidate roots (env, our prefix, Brew, system, MacPorts share)
  CAND_ROOTS="
${PDK_ROOT-}
...
"
  ...
  # If still not found, (re)build from source and re-scan once
  if [ -z "$FOUND_RC" ]; then
  ...
  [ -n "$FOUND_RC" ] || fail "Could not locate sky130A.magicrc anywhere. The open_pdks build likely failed."
...
main() {
  ...
  ensure_homebrew
  install_brew_deps
  build_magic_from_source          # always build Cocoa Tk Magic for consistency
  ensure_magic_on_path             # make "magic" resolve to our build
  build_open_pdks
  ...
  write_magic_rc
  install_magic_wrapper
  ensure_sky130_loaded_and_fix     # self-heal if needed
